							 ...
							 int node )
{
	...
	/*	float distance = 1.0f + sqrtf( SQR((float)centerX - gViewPosition[0]) +
		...
		float distance = 1.0f + sqrtf( SQR((float)centerX - m_ViewHotX) +
			SQR((float)centerY - m_ViewHotY) );

		
		//float distance = 1.0f + sqrtf( SQR((float)centerX - gCameraPos[0]) +
		//	SQR((float)centerY - gCameraPos[2]) );
...
//
void Patch::Tessellate(float viewHotX, float viewHotY)
{
	m_ViewHotX = (int)viewHotX;
	m_ViewHotY = (int)viewHotY;
...
void Landscape::Reset()
{
	...
	int eyeX = (leftX + rightX) >> 1;
	int eyeY = (leftY + rightY) >> 1;
...
//
void Landscape::Tessellate(float viewHotX, float viewHotY)
{
	...
	{
		if (patch->isVisibile())
			patch->Tessellate(viewHotX, viewHotY);
	}
}
